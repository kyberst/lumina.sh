    ...
    const handleSubmit = () => {
        for (const req of requests) {
            if (!values[req.key] && !req.defaultValue) {
                toast.error(`${t('envRequest.missing', 'workspace')} ${req.key}`);
                return;
            }
        }
        onSave(values);
    ...
    if (saved) {
        return (
            <div className="bg-[#ffffbb]/50 border border-[#ffc93a]/50 p-3 rounded-xl text-xs mt-2 flex items-center gap-2">
                ...
                <span className="text-[#ff7e15]">{t('envRequest.saved', 'workspace')}</span>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white border border-[#ffc93a] p-5 rounded-2xl mt-3 shadow-lg shadow-[#ffc93a]/10 animate-in zoom-in-95 relative overflow-hidden">
            ...
            <div className="space-y-4">
                {requests.map(req => (
                    <div key={req.key}>
                        ...
                        {req.type === 'select' && req.options ? (
                            <select 
                                className="w-full bg-[#ffffbb]/30 border border-[#ffc93a]/50 hover:border-[#ff7e15] rounded-xl p-2.5 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-[#ffff7e] transition-all cursor-pointer font-bold"
                                value={values[req.key] || ''}
                                onChange={e => setValues({...values, [req.key]: e.target.value})}
                            >
                                <option value="">{t('envRequest.selectOption', 'workspace')}</option>
                            ...
                            <div className="relative group">
                                <input 
                                    ...
                                    className="w-full bg-[#ffffbb]/30 border border-[#ffc93a]/50 hover:border-[#ff7e15] rounded-xl p-2.5 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-[#ffff7e] transition-all font-mono placeholder:text-slate-400"
                ...
                >
                    {t('envRequest.encryptSave', 'workspace')}
                </button>
            </div>
        </div>
    );
