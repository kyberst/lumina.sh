    ...
    <div className="absolute top-0 left-0 bottom-0 w-full bg-slate-50 border-r border-slate-200 z-10 shadow-2xl animate-in slide-in-from-left-full duration-300 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <Section title={t('elementEditor.typography', 'builder')}>
            ...
            <div>
                <label className="text-sm font-medium text-slate-600">{t('elementEditor.size', 'builder')}</label>
                <input type="text" placeholder={t('elementEditor.placeholder.size', 'builder')} value={fields.fontSize} onChange={e => setFields(f=>({...f, fontSize: e.target.value}))} className="shadcn-input h-10 text-sm mt-1" />
            </div>
        </Section>
        ...
        <Section title={t('elementEditor.spacing', 'builder')}>
         ...
         <Section title={t('elementEditor.contentAttr', 'builder')}>
            <div>
                <label className="text-sm font-medium text-slate-600">{t('elementEditor.textContent', 'builder')}</label>
                <input type="text" placeholder={t('elementEditor.placeholder.text', 'builder')} value={fields.textContent} onChange={e => setFields(f=>({...f, textContent: e.target.value}))} className="shadcn-input h-10 text-sm mt-1" />
            </div>
            <div>
                <label className="text-sm font-medium text-slate-600">{t('elementEditor.href', 'builder')}</label>
                <input type="text" placeholder={t('elementEditor.placeholder.href', 'builder')} value={fields.href} onChange={e => setFields(f=>({...f, href: e.target.value}))} className="shadcn-input h-10 text-sm mt-1" />
            </div>
        </Section>
      </div>

      <div className="p-4 border-t border-slate-200 bg-white mt-auto shrink-0">
        <button onClick={handleApply} className="shadcn-btn shadcn-btn-primary w-full h-10 text-sm">{t('elementEditor.apply', 'builder')}</button>
      </div>
    </div>
  );
};
