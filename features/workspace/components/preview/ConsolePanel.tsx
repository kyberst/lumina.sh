    ...
    return (
        <div className="absolute bottom-14 right-4 w-80 sm:w-96 max-h-[400px] bg-slate-900 rounded-xl shadow-2xl border border-slate-700 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 z-50">
            <div className="bg-slate-800 p-2 flex justify-between items-center border-b border-slate-700">
                <span className="text-[10px] uppercase font-bold text-slate-400 px-2">{t('systemLogs', 'workspace')}</span>
                <div className="flex gap-2">
                    <button onClick={onClear} className="text-[10px] text-slate-400 hover:text-white">{t('clear', 'common')}</button>
                    <button onClick={onClose} className="text-slate-400 hover:text-white">×</button>
                </div>
            </div>
            <div className="flex-1 overflow-y-auto p-2 font-mono text-xs space-y-1">
                {logs.length === 0 && <div className="text-slate-500 italic p-2">{t('noLogs', 'workspace')}</div>}
                {logs.map((log, i) => (
                    <div key={i} className={`flex flex-col p-1.5 border-b border-slate-800/50 ${log.type === 'error' ? 'bg-red-900/10' : ''}`}>
                        ...
                        {log.stack && (
                            <pre className="text-[10px] text-slate-500 mt-1 whitespace-pre-wrap pl-4 font-sans">
                                {log.stack.split('\n').slice(1).join('\n')}
                            </pre>
                        )}
                        {log.source && onNavigate && (
                            ...
                            >
                                <span>{t('at', 'workspace')} {log.source.file}:{log.source.line}</span>
                            </button>
                        )}
                    </div>
                ))}
