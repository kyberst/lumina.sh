...
export const PreviewToolbar: React.FC<Props> = ({ deviceMode, setDeviceMode, depStatus, isSelectionModeActive, onToggleSelectionMode, onToggleFullscreen, isFullscreen }) => {
    return (
        <div className="bg-white border-b border-slate-200 p-2 flex justify-center items-center gap-2 sm:gap-4 shrink-0 relative">
             <div className="flex gap-1 bg-slate-100 p-1 rounded-md">
                {(['mobile', 'tablet', 'desktop'] as const).map(mode => (
                    <button key={mode} onClick={() => setDeviceMode(mode)} className={`p-2 rounded transition-colors ${deviceMode === mode ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
             ...
             <div className="flex gap-1">
                <button onClick={() => onToggleSelectionMode()} title="Select Element" className={`p-2 rounded-md transition-colors ${isSelectionModeActive ? 'bg-indigo-100 text-indigo-600' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600'}`}>
                 ...
                 <button onClick={onToggleFullscreen} title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"} className="p-2 rounded-md text-slate-400 hover:bg-slate-50 hover:text-slate-600">
                     {isFullscreen ? (
