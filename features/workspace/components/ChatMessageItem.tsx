...
export const ChatMessageItem: React.FC<Props> = ({ msg, prevSnapshot, onRollback, onEnvVarSave }) => {
    ...
    // Helper to find original file from previous snapshot
        ...
        <div className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} group flex-col mb-6 animate-in slide-in-from-bottom-2`}>
            <div className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} w-full`}>
                <div className={`max-w-[95%] sm:max-w-[90%] rounded-2xl text-xs sm:text-sm shadow-md relative overflow-hidden border transition-all hover:shadow-lg ${isModel ? 'border-slate-200 bg-white' : 'border-indigo-600 bg-indigo-600 text-white'}`}>
                    ...
                    {/* --- TASK CARD HEADER (Only for Model) --- */}
                    {isModel && (
                    ...
                    <div className={`p-5 ${collapsed ? 'hidden' : ''}`}>
                        ...
                        {/* Friendly Warning for Justified Deletion */}
                        ...
                        {displayReasoning && isModel && (
                            <div className={`mb-5 rounded-xl overflow-hidden border border-slate-200 bg-slate-50/50`}>
                                ...
                                <div className="hidden p-4 pt-2 text-xs text-slate-600 font-medium whitespace-pre-wrap max-h-60 overflow-y-auto custom-scrollbar border-t border-slate-200/50">
                                    {displayReasoning}
                                </div>
                            </div>
                        )}
                        ...
                        {/* Modified Files - Task Card Style */}
                            ...
                            <div className="mt-6 pt-2">
                                ...
                                <div className="space-y-3">
                                    {msg.modifiedFiles.map(filename => {
                                        ...
                                        // If content exists in snapshot, use FileCard
                                        if (fileContent) {
                                            return (
                                                <FileCard 
                                                    key={filename} 
                        ...
                        {/* --- APPROVAL ACTION BUTTONS (Only Model) --- */}
                        {isModel && (
                            <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
                                ...
                                {msg.snapshot && (
                                    <button 
                                        onClick={() => onRollback(msg.snapshot)}
