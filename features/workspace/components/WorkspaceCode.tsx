    ...
    <div className="w-full h-full flex">
                ...
                {toggleContextFile && setAllContextFiles && (
                    <div className="flex items-center justify-between">
                        <label className="flex items-center gap-2 cursor-pointer text-[10px] font-semibold text-indigo-600 hover:text-indigo-800">
                            ...
                            />
                            {isAllSelected ? t('deselectAll', 'workspace') : t('selectAll', 'workspace')}
                        </label>
                            ...
                            {toggleContextFile && (
                                <div className="pl-3 pr-1">
                                    <input 
                                        ...
                                        title={t('contextTooltip', 'workspace')}
                                    />
                                </div>
                            )}
                            <button 
        ...
        <div className="flex-1 relative bg-[#1e1e1e]">
            {selectedFile ? (
                ...
                <div className="flex items-center justify-center h-full text-slate-500 text-sm">{t('selectFileToEdit', 'workspace')}</div>
            )}
            ...
            {readOnly && (
                <div className="absolute top-4 right-4 z-20 flex items-center gap-2 bg-indigo-600/90 text-white px-3 py-1.5 rounded-full shadow-lg backdrop-blur-sm animate-pulse">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wide">{t('aiEditing', 'workspace')}</span>
                </div>
            )}
