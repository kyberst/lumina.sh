                ...
                <div className="p-4 border rounded-xl bg-slate-50 space-y-3 animate-in fade-in">
                    <div className="space-y-2">
                        <div>
                            ...
                            <input className="shadcn-input" placeholder={t('serverNamePlaceholder', 'settings')} value={editing.name} onChange={e => setEditing({...editing, name: e.target.value})} />
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                             <div className="col-span-1">
                                ...
                                <select className="shadcn-input" value={editing.type} onChange={e => setEditing({...editing, type: e.target.value as any})}>
                                    <option value="stdio">{t('stdio', 'settings')}</option>
                                    <option value="websocket">{t('websocket', 'settings')}</option>
                                </select>
                             </div>
                             <div className="col-span-2">
                                <label className="text-xs font-bold uppercase text-slate-500">{t('commandUrl', 'settings')}</label>
                                <input className="shadcn-input" placeholder={editing.type === 'stdio' ? t('commandPlaceholder', 'settings') : "ws://localhost:8080"} value={editing.command} onChange={e => setEditing({...editing, command: e.target.value})} />
                             </div>
                        </div>
                        <div>
                            <label className="text-xs font-bold uppercase text-slate-500">{t('args', 'settings')}</label>
                            <input className="shadcn-input" placeholder={t('argsPlaceholder', 'settings')} value={editing.args?.join(' ')} onChange={e => setEditing({...editing, args: e.target.value.split(' ')})} />
                        </div>
                    </div>
