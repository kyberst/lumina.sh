...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate, onStartTutorial }) => {
  const navItemClass = (view: string) => 
    `relative flex items-center gap-2 text-sm font-semibold transition-all px-3 sm:px-4 py-2 rounded-full overflow-hidden shrink-0 group ${
      currentView === view 
  ...
  return (
    <div className="min-h-screen font-sans flex flex-col relative overflow-hidden bg-background">
      ...
      {/* Navbar - Glassmorphic & Floating - Responsive positioning */}
      <nav className="fixed top-2 left-2 right-2 sm:top-4 sm:left-4 sm:right-4 z-50 rounded-2xl glass-panel h-14 sm:h-16 flex items-center justify-between px-3 sm:px-6 shadow-sm transition-all border border-white/20">
        <div className="flex items-center gap-3 cursor-pointer group shrink-0" onClick={() => onNavigate('builder')}>
          <div className="w-8 h-8 sm:w-9 sm:h-9 bg-foreground text-background rounded-lg shadow-lg flex items-center justify-center transform group-hover:scale-105 transition-all duration-300">
            <span className="font-bold text-base sm:text-lg tracking-tighter">Lu</span>
          </div>
          <span className="text-base sm:text-lg font-bold text-foreground hidden sm:block tracking-tight font-['Plus_Jakarta_Sans']">
        ...
        <div className="flex items-center gap-2 max-w-full overflow-hidden">
          ...
          <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto no-scrollbar py-2 px-1 mask-linear-fade">
            ...
            <div className="w-px h-5 sm:h-6 bg-border mx-1 sm:mx-2 hidden sm:block"></div>

            {onStartTutorial && (
              <button onClick={onStartTutorial} className={`p-2 sm:p-2.5 rounded-full hover:bg-muted text-muted-foreground hover:text-foreground transition-colors shrink-0`} title={t('help', 'nav')}>
            ...
            <button onClick={() => onNavigate('settings')} className={`p-2 sm:p-2.5 rounded-full hover:bg-muted text-muted-foreground hover:text-foreground transition-colors shrink-0 ${currentView === 'settings' ? 'bg-muted text-foreground' : ''}`} title={t('settings', 'nav')}>
      ...
      <main className="relative z-10 pt-20 sm:pt-28 px-3 sm:px-4 pb-12 max-w-7xl mx-auto w-full flex-grow flex flex-col min-h-0">
        {children}
      </main>

      {/* Footer */}
      <footer className="py-6 sm:py-8 text-center border-t border-border bg-background/50 backdrop-blur-sm mt-auto shrink-0 relative z-10">
